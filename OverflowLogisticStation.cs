...

{
    ...
    public class OverflowLogisticStation : BaseUnityPlugin
    {
        ...
        public static void UpdateNeedsPatch(ref StationComponent __instance)
        {
            ...
            {
                if (VeinCollectorOverflow.Value)//如果允许溢出，大矿机的采集速度可能超过每帧1个，这样还是有可能停工，除非每帧修改成max-10这样减的更多一些
                {
                    overflowAmount = 10;
                }
                else//否则返回
            ...
            lock (obj)
            {
                ...
                {
                    if(_this.storage[i].localLogic == ELogisticStorage.Supply && ( !_this.isStellar || _this.storage[i].remoteLogic == ELogisticStorage.Supply) )//对于星际，必须都是供给才允许溢出，对于行星内物流站，则只需要本地供给就允许溢出
                    {
                        _this.storage[i].count = _this.storage[i].count >= _this.storage[i].max ? _this.storage[i].max - overflowAmount : _this.storage[i].count;
                    }
                }
            }
        }
